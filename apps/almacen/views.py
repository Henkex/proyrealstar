...
class SalidaView(LoginRequiredMixin,View,SuccessMessageMixin):
    ...
    def get(self, request):
        ...
        if prod_list.__len__() >0:
            return render(request,self.template_name,{'salidaform':salidaform,'formset':formset,'prod_list':prod_list,'status':True})
        else:
            return render(request,self.template_name,{'salidaform':salidaform,'formset':formset})
    def post(self,request):
...
def InsertarDetalleAlmacen(ingreso_id,almacen_id):
    for p in DetalleIngreso.objects.filter(id_ingreso = ingreso_id).distinct():
